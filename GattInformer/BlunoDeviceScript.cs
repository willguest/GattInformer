...
public class BlunoDeviceScript
{
    ...
    public void OnBluetoothMessage(string message)
    {
        if (message != null)
        {
            ...
            for (int i = 0; i < parts.Length; ++i)
                Debug.WriteLine(string.Format("Part: {0} - {1}", i, parts[i]));
            ...
            {
                Debug.WriteLine("Started Advertising");
            ...
            {
                Debug.WriteLine("Stopped Advertising");
    ...
    public void OnBluetoothData(string deviceAddress, string characteristic, string base64Data)
    {
        if (base64Data != null)
        {
            ...
            if (bytes.Length > 0)
            {
                ...
                Debug.WriteLine("Device: " + deviceAddress + " Characteristic Received: " + characteristic);
    ...
    public void OnPeripheralData(string characteristic, string base64Data)
    {
        if (base64Data != null)
        {
            ...
            if (bytes.Length > 0)
            {
                Debug.WriteLine("Peripheral Received: " + characteristic);
    ...
    public void SubscribeCharacteristic(string name, string service, string characteristicUuid, Action<string> notificationAction, Action<string, byte[]> action)
    {
        characteristicUuid = characteristicUuid.ToLower();

        if (!DidUpdateNotificationStateForCharacteristicAction.ContainsKey(name))
        { DidUpdateNotificationStateForCharacteristicAction[name] = new Dictionary<string, Action<string>>(); }
        DidUpdateNotificationStateForCharacteristicAction[name][characteristicUuid] = notificationAction;

        if (!DidUpdateCharacteristicValueAction.ContainsKey(name))
        { DidUpdateCharacteristicValueAction[name] = new Dictionary<string, Action<string, byte[]>>(); }
        DidUpdateCharacteristicValueAction[name][characteristicUuid] = action;
